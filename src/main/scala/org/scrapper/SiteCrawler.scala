...
class SiteCrawler(supervisor: ActorRef, indexer: ActorRef, downloader: ActorRef) extends Actor{
  val process = "Process next url"

  val scraper = context actorOf Props(new Scraper(indexer, downloader))
